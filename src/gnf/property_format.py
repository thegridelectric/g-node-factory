...
def check_is_algo_address_string_format(candidate: str):
    ...
    Throw SchemaError if algorand AddressType decode(encode(c)) fails.
    ...
    try:
        ...
        raise SchemaError(f"Not AlgoAddressStringFormat: {e}")
...
def check_is_algo_private_key_format(candidate: str) -> bool:
    ...
    try:
        ...
        raise SchemaError(f"Not AlgoSecretKeyFormat: {e} /n {candidate} ")
    return True


def check_is_algo_msg_pack_encoded(candidate: str):
    """Throw SchemaError if algosdk.encoding.future_msg_decode(candidate)
    throws an error"""
    try:
        algosdk.encoding.future_msgpack_decode(candidate)
    except Exception as e:
        raise SchemaError(f"Not AlgoMsgPackEncoded: {e} /n {candidate}")


def check_is_valid_asa_name(candidate: str):
    """Throw SchemaError if not a string or longer than 32 chars"""
    try:
        l = len(candidate)
    except Exception as e:
        raise SchemaError(f"Not ValidAsaName: {e} /n {candidate} ")
    if l > 32:
        raise SchemaError(
...
def check_is_64_bit_hex(candidate):
    if len(candidate) != 8:
        raise SchemaError(f" {candidate} Must be length 8, not {len(candidate)}")
    if not all(c in string.hexdigits for c in candidate):
        raise SchemaError("Must be hex digits")


def check_is_bit(candidate):
    if candidate not in {0, 1}:
        raise SchemaError(f"{e} must be either 0 or 1")
...
def check_is_lrd_alias_format(candidate: str):
    ...
    try:
        ...
        raise SchemaError("Failed to seperate into words with split'.'")
    first_word = x[0]
    ...
    if not candidate.islower():
        raise SchemaError(f"alias must be lowercase. Got '{candidate}'")
...
def check_is_unsigned_short(candidate):
    try:
        ...
        raise SchemaError("requires 0 <= number <= 65535")
...
def check_is_short_integer(candidate):
    try:
        ...
        raise SchemaError("short format requires (-32767 -1) <= number <= 32767")


def check_is_uuid_canonical_textual(candidate):
    """
    Raises:
        SchemaError: if not of the format "f7d517d9-7d3f-47bd-bd5c-ee04f636da6f"
    """
    try:
        x = candidate.split("-")
    except AttributeError as e:
        raise SchemaError(f"Failed to split on -: {e}")
    if len(x) != 5:
        raise SchemaError(f"Did not have 5 words")
    for hex_word in x:
        try:
            y = int(hex_word, 16)
        except ValueError:
            raise SchemaError("Words are not all hex")
    if len(x[0]) != 8:
        raise SchemaError("Word 0  not of length 8")
    if len(x[1]) != 4:
        raise SchemaError("Word 1 not of length 4")
    if len(x[2]) != 4:
        raise SchemaError("Word 2 not of length 4")
    if len(x[3]) != 4:
        raise SchemaError("Word 3 not of length 4")
    if len(x[4]) != 12:
        raise SchemaError("Word 4 not of length 12")


def check_world_alias_matches_universe(g_node_alias: str, universe: str):
    """
    Raises:
        SchemaError: if g_node_alias is not LRD format or if first word does not match universe
    """

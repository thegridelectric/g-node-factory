        ...
        <FileSet>
            <FileSetFiles>
                ...
                <xsl:variable name="full-type-name" select="Alias"/>
                    ...
                    <FileSetFile>
                                <xsl:element name="RelativePath"><xsl:text>../../../src/gnf/types/</xsl:text>
...
<xsl:text>
from gnf.data_classes.</xsl:text>
<xsl:call-template name="python-case">
    <xsl:with-param name="camel-case-text" select="translate(DataClass,'.','_')"  />
</xsl:call-template>
...
<xsl:text>
from gnf.enums import </xsl:text>
<xsl:call-template name="nt-case">
...
def check_is_market_type_name_lrd_format(v: str) -> None:
    from gnf.enums import MarketTypeName
    try:
        x = v.split(".")
...
def check_is_market_slot_name_lrd_format(v: str) -> None:
    ...
    """
    from gnf.data_classes.market_type import MarketType
    try:
        x = v.split(".")
    except AttributeError:
        raise ValueError(f"{v} failed to split on '.'")
